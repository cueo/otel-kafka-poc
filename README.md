...
![OpenTelemetry Architecture](https://opentelemetry.io/docs/collector/img/otel-collector.svg)
...
Any otel (OpenTelemetry) component is defined using a `metadata.yaml` file.
...
The entire schema is defined [here](receivers/vmreceiver/metadata.yaml).
...
Now we are ready to write our receiver code. We will use [vmstat](https://man7.org/linux/man-pages/man8/vmstat.8.html) to scrape the virtual memory metrics.
...
Let's add the Prometheus exporter to our pipeline.
